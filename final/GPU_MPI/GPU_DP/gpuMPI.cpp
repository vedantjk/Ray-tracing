...
int main(int argc, char *argv[])
{
    ...
    double *G = (double *)malloc(sizeof(double) * (n + 1) * (n + 1));
    double *localG = (double *)malloc(sizeof(double) * (n + 1) * (n + 1));

    computeGPU(localG, Nrays / size, n, threadsPerBlock, numBlocks);

    // Reduce localG data across all processes
    MPI_Reduce(localG, G, (n + 1) * (n + 1), MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    if (rank == 0)
    {
